...
namespace WindowsFormsApplication1
{
    public partial class Form1 : Form
    {
            ...
            if (mouseEventArgs != null)
            {
                ...
                if (CurrentTool == "floodfill") {
                    if (mouseEventArgs.Button == MouseButtons.Left) {
                        FloodFillPixel(x, y, spritepixel[CurrentBank, CurrentSprite, x, y], colorLeft, colorRight);
                    }
                    if (mouseEventArgs.Button == MouseButtons.Right)
                    {
                        //VbX.MsgBox(spritepixel[CurrentBank, CurrentSprite, x, y].ToString());
                        // VbX.MsgBox(colorRight.ToString());
                        FloodFillPixel(x, y, spritepixel[CurrentBank, CurrentSprite, x, y], colorRight, colorLeft);
                    }
        ...
        /************************************************************************************************************************************************/
        private void TileCopy(int x, int y, MouseEventArgs mouseEventArgs)  // copy 8x8 square to somewhere else
        {
...
/************************************************************************************************************************************************/
        private void ColorSwap(int x,int y,MouseEventArgs mouseEventArgs) {     // Swap two colors
...
/************************************************************************************************************************************************/
        private void FloodFillPixel(int xx,int yy,int changecolor,byte color1,byte color2){
            ForceBackup();
            int chkr = 0;
            if (cboCheckMode.Text == "1/3") chkr = 1;
            if (cboCheckMode.Text == "2/3") chkr = 2;
            ...
            if (chkr > 0 && changecolor == color2) return;
               ...
               while (myStack.Count > 0)
               {
                   ...
                   if (spritepixel[CurrentBank, CurrentSprite, xx, yy] == changecolor)
                   {
                       ...
                       spritepixel[CurrentBank, CurrentSprite, xx, yy] = checkerize(xx, yy,  color1,  color2, chkr);    // Set pixel
...
/************************************************************************************************************************************************/
        ...
        private void SaveCPCRawBmp_Click(object sender, EventArgs e)
        {
            ...
            if (rdoDispEnt256.Checked)
            {
                CPC_SaveRAW_256Color(BW, CurrentSprite, 1);
...
/************************************************************************************************************************************************/
        private void dosave(String filename)
        {
            ...
            SR.Write("SpriteSize," + maxSpriteSizeX.ToString() +","+ maxSpriteSizeY.ToString() + nl);
        ...
        private void DoLoadFile(string File, bool loadpixels, bool loadbackground, bool loadpalette, bool loadsettings)
        {
            ...
            while (ss.GetItem(lin2, ",", 0).ToLower() != "eof" && !SR.EndOfStream)
            {
                ...
                if (command == "spritesize" && loadsettings)
                {
                    ...
                    if (maxSpriteSizeX == 512 && maxSpriteSizeY == 256) {picPreview.Height = 128;
        ...
        {
            if (CurrentBank < (BankCount-1)) CurrentBank = CurrentBank + 1;
...
/************************************************************************************************************************************************/
        private void simple4x4x4ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            for (int i = 0; i <= 255; i++)
            {
                ...
                if (i >= 48 && i < 64) {
                    int c = i-48;
                    if (c <= 15) setpalette(i, DP.Fleshtones[c]); else
...
/************************************************************************************************************************************************/
        ...
        {
            SaveGridSprites(VbX.CInt(txtHspriteW.Text), VbX.CInt(txtHspriteH.Text), 4);
        }
/************************************************************************************************************************************************/
        public void SaveGridSprites(int Xsize,int Ysize,int BitDepth) {

            SaveFileDialog fd = new SaveFileDialog();
            fd.Filter = "Bitmap Sprites (*.SPR)|*.SPR";
            DialogResult dr = fd.ShowDialog();
            if (dr != DialogResult.OK) return;
...
/************************************************************************************************************************************************/
        ...
        {
            SaveGridSprites(VbX.CInt(txtHspriteW.Text), VbX.CInt(txtHspriteH.Text), 8);
        }
/************************************************************************************************************************************************/
